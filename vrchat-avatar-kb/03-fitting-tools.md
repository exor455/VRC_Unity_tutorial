...
衣装の位置・回転・スケールをボーン単位で調整する老舗の着せ付け補助ツール。**AAOが1.5.4で明示的に互換対応を追加**した実績があり(KB [AAO バージョン履歴](09-avatar-optimizer-changelog.md))、NDMFエコシステムでの使用実績が長い。もちふぃった～登場後は「変換まではいらない、位置合わせだけしたい」ケース向け。
