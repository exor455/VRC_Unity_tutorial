...
## スクリプト実行ノート(Manual Bake の自動実行・実測)

エディタスクリプトから Manual Bake 相当のビルドを実行する場合の実測知見(Unity 2022.3.22f1 / NDMF 1.14.0 / MA 1.17.1)。

### 呼び出し

`nadena.dev.ndmf.AvatarProcessor.ProcessAvatarUI(GameObject)`(public static)をリフレクション経由で呼ぶ。

**アバター本体を直接渡すこと。Instantiate した clone を渡すと二重 Clone が作られる**: `Object.Instantiate(avatar)` で作った clone を渡すと、NDMF 内部でさらに Instantiate が行われ `<名前>(Clone)(Clone)` が作られ、生成物の検索が複雑になる。本体を渡した場合は `<アバター名>(Clone)` がシーン上に現れるので、`GameObject.Find("<アバター名>(Clone)")` で取得できる。

### 後始末

検査後、Clone を取得して `Object.DestroyImmediate` で削除する(シーンを汚さない)。Bake が途中で例外失敗しても Clone が残ることが多く検査は可能。

### 例外の切り分け

- **アバター固有の既存不良との切り分け**: 対象アバター固有の不良(例: PhysBone のコンポーネントリストに null エントリ)があると `PhysbonesBlockerPluginPass` で `ArgumentException` が出るが、検証対象とは無関係なことがある。エラーの発生タイミングと対象を確認して切り分ける
- **null motion State による bake 例外**: Manual Bake またはビルドが `VirtualClip.Commit` 系の例外で途中停止する場合、対象アバターの FX 等 AnimatorController に **motion が null の State**(制作者が表情差し替え用に残したプレースホルダー State 等)が含まれていることがある。切り分け手順: FX 等を一時的に空の AnimatorController に差し替えて bake が通るか確認する。通れば null motion State が原因。恒久対処は当該 State に空 AnimationClip を割り当てるか State を除去する。検証後は元コントローラを必ず復元すること
...
- **Manual Bake またはビルドが `VirtualClip.Commit` 系の例外で途中停止する**: アバターの AnimatorController に motion が null の State が含まれている可能性。上記「スクリプト実行ノート §例外の切り分け」参照
