...
## ステータス: v1 完成(2026-07-11)

主要ツール(NDMF / Modular Avatar / TexTransTool / lilToon / Poiyomi / AAO / VRCQuestTools 等)+基礎・ライセンス・Quest対応・問診ガイド・素体差リファレンスまでのカバレッジ駆動の執筆はここで完了とする。以後の追記は**事例駆動**のみ:

1. 実事例(相談・自走検証)でKBが解けなかったケースの還元(調査手順を優先)
2. ツールのバージョン更新に伴う changelog / 挙動変化の反映
3. 実測でKB記述と食い違いが見つかった場合の訂正

網羅率ではなく「実事例の正答率」を評価軸とする。
